...
final class FlumeEventQueue {
    ...
    FlumeEventQueue(EventQueueBackingStore backingStore, File inflightTakesFile, File inflightPutsFile, File queueSetDBDir) throws Exception {
        ...
        Preconditions.checkArgument(backingStore.getCapacity() > 0, "Capacity must be greater than zero");
        ...
        queueSet = db.createHashSet("QueueSet " + " - " + backingStore.getName()).make();
    ...
    protected synchronized long remove(int index, long transactionID) {
        if (index < 0 || index > backingStore.getSize() - 1) {
            throw new IndexOutOfBoundsException("index = " + index + ", queueSize " + backingStore.getSize() + " " + channelNameDescriptor);
        }
    ...
    class InflightEventWrapper {
        ...
        public InflightEventWrapper(File inflightEventsFile) throws Exception {
            if (!inflightEventsFile.exists()) {
                Preconditions.checkState(inflightEventsFile.createNewFile(), "Could not" + "create inflight events file: " + inflightEventsFile.getCanonicalPath());
            }
        ...
        public void addEvent(Long transactionID, Long pointer) {
            ...
            inflightFileIDs.put(transactionID, FlumeEventPointer.fromLong(pointer).getFileID());
            syncRequired = true;
        }
        ...
        public void serializeAndWrite() throws Exception {
            ...
            try {
                ...
                Preconditions.checkState(file.length() == expectedFileSize, "Expected File size of inflight events file does not match the " + "current file size. Checkpoint is incomplete.");
                file.seek(0);
                ...
                for (Long txnID : inflightEvents.keySet()) {
                    ...
                    LOG.debug("Number of events inserted into " + "inflights file: " + String.valueOf(pointers.size()) + " file: " + inflightEventsFile.getCanonicalPath());
        ...
        public SetMultimap<Long, Long> deserialize() throws IOException, BadCheckpointException {
            ...
            file.read(checksum);
            ByteBuffer buffer = ByteBuffer.allocate((int) (file.length() - file.getFilePointer()));
